...
import { getAuth, login, makeLoginUrl, makeLogoutUrl } from '../utils/auth';
import {
  createContext,
  PropsWithChildren,
  useCallback,
...
type AuthContextProps = {
  ...
  makeLoginUrl: () => string;
  login: (accessToken: string, idToken: string, state: string) => void;
...
export function AuthProvider(props: PropsWithChildren) {
  const makeLogin = useCallback(
    (accessToken: string, idToken: string, state: string) => {
      const authData = login(accessToken, idToken, state);
      setData((prev) => ({
        auth: authData,
